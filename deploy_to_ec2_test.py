...
class DeployToEC2Test(unittest.TestCase):
    ...
    @patch('deploy_to_ec2.open_connection', return_class=MockClient)
    @patch('deploy_to_ec2.deploy_artifact')
    def test_start_deployment_error(self, mock_boto3, mock_client):
        """ Test for exit code and logging if deployment does not start """
        # GIVEN
        sys.stdout = StringIO()
        mock_client.return_class = MockClient()
        mock_boto3.side_effect = ClientError(
            {
                "Error": {
                    "Code": "MockedException"
    ...
    def test_deployment_succeeded(self, deploy_artifact, get_status, mocked_client):
        """ Test for exit code and logging if deployment runs okay """
        # GIVEN
        sys.stdout = StringIO()
        mocked_client.return_class = MockClient
    ...
    def test_deployment_failed(self, deploy_artifact, get_status, mock_client):
    ...
    def test_deployment_stopped(self, deploy_artifact, get_status, mock_client):
